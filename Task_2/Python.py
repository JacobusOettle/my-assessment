...
def question_1(df_balances):
    ...
    # Get the ratio of the repayments for each month
    repayment_ratios = np.array(df_balances['ActualRepayment'] / df_balances['ScheduledRepayment'])

    # Reshape the repayment_ratios array into a 12-element (months) wide array for each unique LoanID
    repayment_ratios = repayment_ratios.reshape(-1, 12)
    
    # Create a boolean array indicating if any value in each row is paid less than the full ammount
    is_defaulted = (repayment_ratios < 1).any(axis=1)
    
    # Calculate the percentage of defaulted loans
    default_rate_percent = np.mean(is_defaulted)*100
...
def question_2(df_scheduled, df_balances):
    ...
    # Get the ratio of each repayment
    repayment_ratios = np.array(df_balances['ActualRepayment'] / df_balances['ScheduledRepayment'])
    
    # Reshape the array such that each row contain the repayment ratios of each month (columns) for each loan (row)
    repayment_ratios = repayment_ratios.reshape(-1, 12)
    
    # Calculate the average repayment ratio for each loan (row)
    avg_repayment_ratios = np.mean(repayment_ratios, axis=1)
    
    # Create a int array from a boolean condition indicating which rows have a ratio less than (1-0.15 = 0.85) which would indicate a type 2 default
    is_defaulted = (avg_repayment_ratios < 0.85).astype(int)
...
def question_3(df_balances):
    ...
    # Calculate the unscheduled principal for each month
    unscheduled_principal = df_balances['ActualRepayment'].to_numpy() - df_balances['ScheduledRepayment'].to_numpy()
    
    # Calculate the SMM for each month
    loan_balance_start = df_balances['LoanBalanceStart'].to_numpy()
    smm = unscheduled_principal / loan_balance_start
    
    # Calculate the geometric mean of the SMM values
    smm_mean = (np.prod(1 + smm.reshape(-1, 12), axis=1) ** (1/12)) - 1
    smm_mean = np.mean(smm_mean)
    
    # Calculate the annualized portfolio CPR
...
def question_4(df_balances):
    ...
    # Calculate the total loan balance at the end of the first year
    total_loan_balance = df_balances.groupby('LoanID')['LoanBalanceEnd'].last().sum()

    # Question 2's type 2 default definition provides a better overview of long term performance compared to Questions 1's type 1 default
    # Since the calulcation is for predicted loss for the second year, a long term performance indicator would be better suited.
    probability_of_default = 0.012
    recovery_rate = 0.8
    
    # Calculate the predicted total loss for the second year
